...
const opening: FC<openingProps> = ({ days }) => {
  ...
  // tRPC
  const { mutate: saveOpeningHrs, status } =
  ...
  const { data: closeDays, refetch } = api.opening.getClosedDays.useQuery();

  const dayIsClosed =
    selectedDate && closeDays?.includes(formatISO(selectedDate));
